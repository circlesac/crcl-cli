...
package_root="$(mktemp -d "${TMPDIR:-/tmp}/crcl-deb.XXXXXX")"
trap 'rm -rf "$package_root"' EXIT
chmod 0755 "$package_root"

install -d -m 0755 "$package_root/DEBIAN" "$package_root/usr/bin"
install -m 0755 "$binary" "$package_root/usr/bin/crcl"
...
Description: Manage Circles organizations, API keys, and authentication
CONTROL

install -d -m 0755 "$output_dir"
dpkg-deb --build --root-owner-group \
  "$package_root" \
  "$output_dir/crcl_${version}_${arch}.deb"
